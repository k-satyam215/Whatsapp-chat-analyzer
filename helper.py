...
def most_busy_user(df):
    ...
    df=round((df['user'].value_counts()/df.shape[0])*100,ndigits=2).reset_index().rename(columns={'count':'percent'})
    return x,df

def create_wordcloud(selected_use,df):

    f=open('stop_hinglish.txt','r',encoding='utf-8')
    stop_words=f.read()

    if selected_use!='Overall':
        df=df[df['user']==selected_use]
